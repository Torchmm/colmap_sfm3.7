...
void GlobalUtil::SelectDisplay()
{
	...
	// 分配足够的空间来存储宽字符字符串
	int len = MultiByteToWideChar(CP_ACP, 0, _WindowDisplay, -1, NULL, 0);
	wchar_t* wideDriverName = new wchar_t[len];
	MultiByteToWideChar(CP_ACP, 0, _WindowDisplay, -1, wideDriverName, len);

	HDC hdc = CreateDC((wideDriverName), (wideDriverName), NULL, NULL);
	_WindowDisplay = NULL;
	if(hdc == NULL)
	{
